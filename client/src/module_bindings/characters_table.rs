...
/// Extension trait for access to the table `characters`.
///
/// Implemented for [`super::RemoteTables`].
pub trait CharactersTableAccess {
    #[allow(non_snake_case)]
    /// Obtain a [`CharactersTableHandle`], which mediates access to the table `characters`.
...
impl CharactersTableAccess for super::RemoteTables {
    fn characters(&self) -> CharactersTableHandle<'_> {
        CharactersTableHandle {
            imp: self.imp.get_table::<Character>("characters"),
...
pub(super) fn register_table(client_cache: &mut __sdk::ClientCache<super::RemoteModule>) {
    let _table = client_cache.get_or_make_table::<Character>("characters");
...
/// Access to the `character_id` unique index on the table `characters`,
/// which allows point queries on the field of the same name
/// via the [`CharactersCharacterIdUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.characters().character_id().find(...)`.
...
impl<'ctx> CharactersTableHandle<'ctx> {
    /// Get a handle on the `character_id` unique index on the table `characters`.
...
/// Access to the `user_id` unique index on the table `characters`,
/// which allows point queries on the field of the same name
/// via the [`CharactersUserIdUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.characters().user_id().find(...)`.
...
impl<'ctx> CharactersTableHandle<'ctx> {
    /// Get a handle on the `user_id` unique index on the table `characters`.
