...
pub async fn renew(
    ...
    let user_agent = headers
        .get("USER_AGENT")
        .ok_or_else(|| AppError(anyhow!("No auth agent")))?
        .to_str()
        .map_err(|_| AppError(anyhow!("Invalid auth agent")))?;
    ...
    let user_id = Uuid::parse_str(&claims.claims.sub).expect("Unable to parse auth id");
